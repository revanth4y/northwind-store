    ...
    <header className="sticky top-0 z-50 border-b border-base-300 bg-base-100/95 shadow-sm backdrop-blur-md">
      <div className="navbar mx-auto min-h-14 max-w-7xl px-4 py-2.5 md:px-6 md:py-3">
        ...
        <nav className="flex items-center gap-1 md:gap-1.5">
          ...
          {/* Video Meet — available to all visitors */}
          <Link to="/meet" className="btn btn-ghost gap-2 font-medium">
            <VideoIcon className="size-6 opacity-90" aria-hidden />
          ...
          <Show when="signed-in">
            ...
            <Link to="/support" className="btn btn-ghost gap-2 font-medium">
              <TicketIcon className="size-6 opacity-90" aria-hidden />
              <span className="hidden sm:inline">Support</span>
            </Link>

            {(role === "admin" || role === "support") ? (
              <Link to="/admin/tickets" className="btn btn-ghost gap-2 font-medium text-secondary">
                <SettingsIcon className="size-6" aria-hidden />
                <span className="hidden sm:inline">{role === "admin" ? "Admin" : "Queue"}</span>
              </Link>
            ) : null}

            {role === "admin" ? (
              <Link to="/admin" className="btn btn-ghost gap-2 font-medium text-accent">
            ...
            {role === "admin" ? (
              <Link to="/admin/shipments" className="btn btn-ghost gap-2 font-medium text-primary">
          ...
          <Link
            to="/cart"
            className="btn btn-ghost indicator gap-2 font-medium"
            aria-label={cartCount > 0 ? `Cart, ${cartCount} items` : "Cart"}
          >
            {cartCount > 0 ? (
              <span className="indicator-item badge badge-primary badge-sm min-w-2 px-1.5 font-sans text-xs tabular-nums">
