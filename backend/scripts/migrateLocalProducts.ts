/**
 * migrateLocalProducts.ts
 *
 * Upserts the entire local product catalog into PostgreSQL.
 * Safe to run multiple times — uses ON CONFLICT (slug) DO UPDATE.
 * Does NOT touch imageUrl or imageKitFileId for products that already
 * have a Cloudinary URL set (detected by checking for "res.cloudinary.com").
 *
 * Usage:
 *   npx tsx scripts/migrateLocalProducts.ts
 */
...
// ── Catalog ───────────────────────────────────────────────────────────────────
// All imageUrls here are Unsplash CDN links — fully public and permanent.
// If a product already has a Cloudinary URL in the DB it will be preserved.
...
async function main() {
  console.log(`\n🚀  Migrating ${CATALOG.length} products to PostgreSQL…\n`);
  ...
  for (const p of CATALOG) {
    ...
    if (existing) {
      const hasCloudinaryImage =
        existing.imageUrl?.includes("res.cloudinary.com") ?? false;
      ...
      await db
        ...
        .set({
          ...
          currency: "usd",
          active: true,
          // Preserve Cloudinary URLs — only replace if still on Unsplash/local
          ...(hasCloudinaryImage ? {} : { imageUrl: p.imageUrl }),
        })
        .where(eq(products.slug, p.slug));

      hasCloudinaryImage
        ? skipped++ && console.log(`  ⏭  ${p.slug} — Cloudinary image preserved`)
        : updated++ && console.log(`  ✏️  ${p.slug} — updated`);
    } else {
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Inserted : ${inserted}
  Updated  : ${updated}
  Skipped  : ${skipped} (Cloudinary URLs preserved)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
